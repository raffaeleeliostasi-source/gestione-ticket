...
def get_cookie_controller():
    ...
    Il valore COOKIES_PASSWORD deve essere presente
        ...
        if submit:
            ...
            if ricordami:
                try:
                    ...
                    if not cookies.ready():
                        st.warning(
                            "Accesso effettuato, ma il salvataggio "
                            "del login automatico non è ancora disponibile."
                        )
                    else:
...
def pagina_dashboard():
    ...
    st.markdown(
        ...
        f'<strong>{len(filtrato)}</strong> ticket visualizzati'
        f'</div>',
        unsafe_allow_html=True,
    )
    ...
    try:
        ...
        if numero_allegati:
            db.registra_evento(
                ...
                "Allegati caricati",
                (
                    f"Caricati {numero_allegati} "
                    f"allegat{'o' if numero_allegati == 1 else 'i'}."
                ),
            )
...
def _mostra_cronologia_interventi(ticket_id):
    ...
    for numero, intervento in enumerate(
        ...
        with st.container(border=True):
            ...
            if foto_path:
                foto = db.scarica_foto_intervento(
                    foto_path
                )
...
def _mostra_storico_ticket(ticket_id):
    ...
    for evento in eventi:
        ...
        utente = (
            _safe(evento.get("utente"))
            or "Sistema"
        )

        azione = (
            _safe(evento.get("azione"))
            or "Evento"
        )
...
def mostra_dettaglio_ticket(ticket_id):
    ...
    c4.markdown(
        f"**TECNICO**<br>{assegnato or 'NON ASSEGNATO'}",
        unsafe_allow_html=True,
    )
    ...
    if interventi:
        ...
        for numero, intervento in enumerate(
            ...
            stato_int = (
                _safe(intervento.get("stato"))
                or "—"
            )

            tecnico_int = (
                _safe(intervento.get("tecnico"))
                or "—"
            )
            ...
            with st.container(border=True):
                ...
                if foto_path:
                    try:
                        ...
                        st.warning(
                            ...
                            "presente ma non visualizzabile."
                        )
                ...
                # Fallback per eventuali record precedenti.
                if (
                    not firma_bytes
                    ...
                    fallback_path = (
                        f"firme/{ticket_id}/"
                        f"intervento_{intervento.get('id')}/"
                        f"firma.png"
                    )
        ...
        if st.button(
            ...
            try:
                ...
                if stato == "Risolto":
                    ...
                    firma_bytes = _firma_da_canvas(
                        canvas_result
                    )

                    if not firma_bytes:
                        st.error(
                            "Inserisci la firma prima "
                            "di risolvere il ticket."
                        )
                        return

                # ------------------------------------------------
                # 1. CREA NUOVO INTERVENTO
                # ------------------------------------------------
                ...
                if firma_bytes is not None:
                    ...
                    # IMPORTANTE:
                    # non facciamo un download immediato
                    # da Storage dopo l'upload.
                    #
                    # Alcune policy/configurazioni Storage
                    # possono impedire la rilettura immediata
                    # anche se l'upload è riuscito.
                    #
                    # salva_firma_intervento() gestisce già
                    # l'eventuale errore reale dell'upload.
...
def pagina_gestione_interventi():
    ...
    if admin:
        ...
        st.caption(
            f"{len(filtrato)} ticket visualizzati"
        )
    ...
    with st.expander(
        ...
        st.caption(
            "I ticket chiusi sono consultabili come storico "
            "e non compaiono più nella parte operativa principale."
        )
...
def pagina_statistiche():
    ...
    st.caption(
        "Cruscotto amministrativo per monitorare andamento, "
        "carichi di lavoro e stato dei ticket."
    )
    ...
    with st.container(
        ...
        with f4:
            ...
            tecnici = sorted(
                [
                    x
                    for x in df["assegnato_a"].unique()
                    if x
                ]
            )
...
def _excel_bytes(df):
    ...
    if "data_chiusura" in export_df.columns:
        export_df[
            "data_chiusura"
        ] = export_df[
            "data_chiusura"
