...
def migrate_database():
    ...
    if "note_tecnico" not in existing:

        execute(
            """
            ALTER TABLE ticket
            ADD COLUMN note_tecnico TEXT DEFAULT ''
            """
        )

    if "creato_il" not in existing:

        execute(
            """
            ALTER TABLE ticket
            ADD COLUMN creato_il TEXT
            """
        )
        ...
        execute(
            ...
            UPDATE ticket
            SET creato_il = ?
            WHERE creato_il IS NULL
            """,
            (now,),
        )

    if "modificato_il" not in existing:

        execute(
            """
            ALTER TABLE ticket
            ADD COLUMN modificato_il TEXT
            """
        )
